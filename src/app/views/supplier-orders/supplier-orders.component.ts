...
export class SupplierOrdersComponent implements OnInit {
  ...
  onCustomerChange(supplier: string): void {
    this.supplierName = supplier;
    console.log('supplierName' ,this.supplierName );
    this.SupplierService.getSupplierIdByName(this.supplierName).subscribe(
      res =>{
        //debugger;
        let data = res['data'];
...
// Get Suppliers
    getSuppliersOrders(){
    this.SupplierService.getSuppliersOrders().subscribe(
      res =>{
        //debugger;
        let data = res['data'];
        this.suppliersOrders = data;
        console.log('suppliersOrders >>>',this.suppliersOrders);
        },
        err =>{
        console.log(err);
      }
  ...
  this.submitted = true;
    if (neworderForm.invalid) {
      ...
      this.toastr.error('الرجاء ملئ جميع الحقول المطلوبة');
       ...
       this.SupplierService.createSupplierOrderForm(createData).subscribe((data) =>{
       this.newUserDialog = false;
       this.getSuppliersOrders();
